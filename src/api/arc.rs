    ...
    thread::spawn(move || {
        ...
        loop {
            match rx.recv() {
                ...
                Ok(Job::Get(ref k, ref cbak)) => match mp.get(k) {
...
impl<K: MKey, V: MVal> ArcMap<K, V> {
    ...
    pub async fn on_do<RT, F>(&mut self, on: K, mut f: F) -> Result<RT, AMapErr>
    where
        F: FnMut(&mut V) -> RT,
    {
        let p = self.get(on)?;
        let mut v = p.lock().await;
        Ok(f(&mut v))
    }
}
